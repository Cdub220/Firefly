...
2. **Plan file** (Chase, ~2 h). Trace the typical floor plan into zones; 7–9 involved floors; ids stay generic (`L22-CORE`, `L22-N`, ...) — no incident names in `src/`. `npm run sim -- --plan <slug>` must run clean; the structure tests (`src/shared/plan.test.ts`) pass on it.
   ...
   **Steps 3–7 DONE Mon Sept 14 2026** — results page `docs/10-incident-replay.md`; `src/incident/{calibration,commander,floors,run}.ts` with tests; `npm run incident`; Case file page and beat 7. Tick is 4 minutes, not 1 (a space's fuel burns 50 ticks; at 1 min/tick nothing would burn long enough). The headline negative result (ours-driven drones dither, Kalman-driven ones contain) is on the results page and in `docs/decisions.md`.
