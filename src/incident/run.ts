...
export function runIncident(opts: { slug: string; seed: number; minutes: number; dir?: string }): IncidentResults {
  ...
  const containment: ContainmentRow[] = [{ driver: 'nobody (uncontrolled)', fireVolumeMinutes: uncontrolled.fireVolumeMinutes, floorsBurned: Object.keys(uncontrolled.firstBurning).map(Number).sort((a, b) => a - b), lastFire: uncontrolled.lastFire, peakBurning: 0, droneDeaths: 0 }];
