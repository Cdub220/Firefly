...
import { runContainment } from './containment';
...
export function hedgeReport(o: HedgeOptions): { text: string; hedgeRate: number; containmentDelta: number } {
  const plan = loadPlan(o.plan);
  const home = plan.resupply[0] ?? plan.spaces[0]!.id;
  const corruption: Omit<CorruptionConfig, 'seed'> = { mode: o.mode, k: o.k, onset: o.onset, target: [plan.ignition[0]!], ambient: plan.ambient };
  ...
  const lines: string[] = [];
  lines.push(`hedge-cp4  plan=${o.plan}  mode=${o.mode}  k=${o.k}  onset=${o.onset}  seed=${o.seed}  ticks=${o.ticks}  target=${plan.ignition[0]}  roster=${HEDGE_ROSTER.map((d) => `${d.id}:${d.class}`).join(',')} at ${home}`);
  lines.push('');
  ...
  return { text: lines.join('\n'), hedgeRate: m.hedgeRate, containmentDelta: run.containmentDelta };
}
...
export function parseHedgeArgs(argv: string[]): HedgeOptions {
  ...
  const mode = get('--mode');
  if (mode !== undefined) o.mode = mode as CorruptionMode;
  for (const [flag, key] of [['--seed', 'seed'], ['--ticks', 'ticks'], ['--k', 'k'], ['--onset', 'onset']] as const) {
    const v = get(flag);
    if (v !== undefined) {
      const n = Number(v);
      if (!Number.isInteger(n) || n < 0) throw new Error(`${flag}: expected a non-negative integer`);
      o[key] = n;
    }
  }
  return o;
...
if (isMain) {
  ...
  writeFileSync('results/hedge-cp4.txt', text + '\n');
  console.log('\nwrote results/hedge-cp4.txt');
}
