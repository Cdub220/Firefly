...
describe('sweep', () => {
  ...
  it('--quick on demo-6 with one mode: expected row count and every metric finite', () => {
    // 1 plan x 1 mode x 3 k x 5 targets x 2 seeds x 4 brains (ours, naive, gated, source kalman)
    expect(result.rows.length).toBe(1 * 1 * 3 * 5 * 2 * 4);
  ...
  it('aggregate averages over seeds and keeps ours/kalman/gated/source on adjacent rows in the summary', () => {
    ...
    expect(whereOursLoses(worse).length).toBe(15);
  ...
  it('resolves targets: ignition, hottest neighbor at onset, farthest by edge hops, random = corruptor picks', () => {
    ...
    expect(resolveTarget(DEMO_PLAN, 'pair', 5, 1)).toEqual(['S3', 'S2']); // ignition + its hottest neighbour, for H5
  ...
  it('parses CLI flags and rejects typos loudly', () => {
    ...
    expect(parseSweepArgs(['--quick', '--targets', 'pair']).targets).toEqual(['pair']);
