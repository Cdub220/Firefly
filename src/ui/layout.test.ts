...
import { CELL_SPACING, computeLayout, LEVEL_SPACING, layoutBounds, layoutFor, MIN_SEPARATION } from './layout';
...
describe('layout', () => {
  it('gives the same layout for the same plan twice, and memoizes per plan object', () => {
    for (const plan of [RING, ...PLAN_NAMES.map(loadPlan)]) {
  ...
  it('no two same-level spaces overlap', () => {
    for (const plan of [RING, ...PLAN_NAMES.map(loadPlan)]) {
      const l = computeLayout(plan);
      for (const a of plan.spaces) {
        for (const b of plan.spaces) {
          if (a.id >= b.id || a.level !== b.level) continue;
          expect(dist2d(l[a.id]!, l[b.id]!)).toBeGreaterThan(Math.min(1.5, MIN_SEPARATION - 0.01));
        }
      }
    }
  });
