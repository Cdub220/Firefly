...
export const SAFE_TEMP = 400; // non-tether drones never sent above this estimate
export const SAFE_TEMP_TETHER = 600;
...
export type AllocatorOptions = {
  /**
   * Spaces that have a trusted reading this tick, so a sensor there adds no information.
   * Default: spaces with a fixed sensor not in belief.suspectSensors, plus spaces holding
   * an alive, linked drone whose reading is not suspect.
   */
  sensed?: Set<SpaceId>;
};
...
export function allocate(
  ...
  const sensed =
    opts.sensed ??
    new Set<SpaceId>([
      ...plan.sensors.filter((f) => !suspect.has(f.id)).map((f) => f.spaceId),
      ...active.filter((d) => !suspect.has(`${d.id}:temp`)).map((d) => d.at),
    ]);
  const { candidates, containment, information } = scoreSpaces(plan, belief, hypotheses, sensed);
  ...
  const safeFor = (d: Drone, id: SpaceId): boolean => estimate(id) <= (d.class === 'tether' ? SAFE_TEMP_TETHER : SAFE_TEMP);
  ...
  const score = (d: Drone, id: SpaceId): number => {
    ...
    return W_CONTAINMENT * (contNow.get(id)?.[d.class] ?? 0) + wInfo(d) * (infoNow.get(id) ?? 0) - W_DISTANCE * dist;
  };
  ...
  while (remaining.size > 0) {
    ...
    const { d } = best;
    let target = best.id;
    let task = TASK_BY_CLASS[d.class];
    ...
    if (prev && prev.task !== 'refill' && prev.task !== 'hold' && prev.goTo !== target && candidates.includes(prev.goTo) && safeFor(d, prev.goTo)) {
      ...
      if (sPrev > 0 && sPrev >= best.s * (1 - HYSTERESIS)) {
        target = prev.goTo;
        task = prev.task;
      }
    }
